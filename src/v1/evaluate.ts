...
export function evaluateExpr(
  expr: MathExpr,
): Undefined | Integer | Fraction | Root {
  switch (expr.type) {
    case "undefined":
      return expr;
    case "integer":
    ...
    case "root":
      return evaluateRoot(expr.degree, expr.radicand);
    default:
...
function evaluateRoot(
  ...
  degree = evaluateExpr(degree);
  radicand = evaluateExpr(radicand);

  if (degree.type == "undefined" || radicand.type == "undefined") {
    return { type: "undefined" };
  }

  if (degree.type == "integer" && radicand.type == "integer") {
    const root = Math.pow(radicand.value, 1 / degree.value);
